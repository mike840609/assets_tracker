...
| B3  | Inspect `date-fns` usage for tree-shaking                                       | Bundle Size | 🟡 Medium | 30 min | ✅ Done — `date-fns` had zero import sites (all date ops use native `Date` API); package removed from `package.json` and `optimizePackageImports`                                                  |
| B4  | Audit `lucide-react` usage                                                      | Bundle Size | 🟡 Medium | 15 min | ✅ Done — all 22 import sites use named destructuring (never the barrel); `lucide-react` already in `optimizePackageImports`; tree-shaking confirmed active                                        |
| B5  | Monitor `recharts` library payload                                              | Bundle Size | 🟡 Medium | 45 min | ✅ Done — all three charts (`TrendChart`, `AllocationChart`, `CurrencyExposureChart`) are `ssr: false` dynamic imports in `lazy-charts.tsx`; recharts chunk is fully split from the initial bundle |
...
| B9  | Evaluate `next-intl` dictionary loading per route                               | Bundle Size | 🟡 Medium | 30 min | ✅ Done — `LocaleProviders` in `layout.tsx` already calls `pickMessages(messages, ["app", "nav", "commandPalette"])`; only 3 of 28 namespaces serialised into the HTML                             |
...
| B12 | Audit `@base-ui/react` tree-shaking                                             | Bundle Size | 🟡 Medium | 30 min | ✅ Done — all 11 UI components use sub-path imports (`@base-ui/react/button` etc.); no barrel `index.ts` in `src/components/ui/`; `@base-ui/react` added to `optimizePackageImports`               |
...
| B14 | Optimize Root Layout Font preloading                                            | Performance | 🟡 Medium | 15 min | ✅ Done — Geist `preload: true` (primary, above-the-fold); GeistMono `preload: false` (code UI only, not critical path); both fonts use `.woff2` latin-subset files                                |
...
Findings sourced on **2026-04-21**. The correct Next.js 16 answer is to walk the rendering ladder SSG → PPR → ISR and only use ISR where the first two don't apply. Items here are additive to V17, V18, V20, V21, V26, V27 (in the Platform section — not duplicates).

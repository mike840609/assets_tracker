...
| R5  | Enforce account/holding ownership on every mutation route                            | Security           | 🔴 High   | 1–2 hrs   | ⚠️ Partial — `holdings/route.ts` + `accounts/[id]/route.ts` covered; `transactions/route.ts` + `cash-transactions/route.ts` missing |

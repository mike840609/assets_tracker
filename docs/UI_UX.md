...
- **Pro broker desktop layout for accounts/holdings** — ✅ Done: `/accounts/[id]` now uses a two-pane desktop layout. Left: sticky `AccountsNavPanel` (`w-44 xl:w-52`, `sticky top-6`) listing all accounts grouped by Assets/Liabilities for quick switching. Right: `HoldingsTable` — a true `<table>` with sticky sortable column headers (Symbol, Name, Type, Qty, Price, Mkt Value, Weight), per-row weight progress bars, and hover-reveal ⋯ actions menu. Mobile: unchanged swipeable card rows. Global container widened from `max-w-6xl` to `max-w-7xl`. Implemented in `accounts-nav-panel.tsx`, `holdings-table.tsx`, `account-detail.tsx`.

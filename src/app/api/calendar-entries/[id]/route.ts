  ...
  async (request, { params }, userId, principal) => {
    try {
      ...
      const merged = createCalendarEntrySchema.safeParse({
        ...
        sourceUrl:
          partial.data.sourceUrl === undefined ? current.sourceUrl : partial.data.sourceUrl,
      });
      ...
      log.error("calendar_entries.update_failed", calendarErrorMetadata(error, principal));
...
export const DELETE = withAuth<IdCtx>(
  async (_request, { params }, userId, principal) => {
    try {
      const { id } = await params;
      const current = await prisma.calendarEntry.findFirst({ where: { id, userId } });
      if (!current) return failure("Not found", 404);
      ...
      log.error("calendar_entries.delete_failed", calendarErrorMetadata(error, principal));

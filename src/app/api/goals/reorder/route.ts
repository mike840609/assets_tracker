...
function invalidateGoalCaches(userId: string, principal: AuthPrincipal) {
  invalidateScopedTag({ globalTag: "goals", userTag: `goals:${userId}`, principal });
}
...
export const PATCH = withAuth(
  async (request, _ctx, userId, principal) => {
    ...
    invalidateGoalCaches(userId, principal);
    return ok({ ok: true });
  },
  { demo: "allow" },
);
